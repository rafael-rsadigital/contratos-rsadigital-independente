...
function GoogleContract({ data, confirmed, confirmDate, valorParcela }: Props & { valorParcela: string }) {
  return (
    <div className="contract-document max-w-3xl mx-auto text-[15px] leading-relaxed">
      <h1 className="text-center text-lg mb-8 tracking-widest font-bold">TERMO DE PRESTAÇÃO DE SERVIÇOS</h1>
      ...
      {/* OBJETO */}
      <h2 className="text-sm font-bold mt-6 mb-2">OBJETO</h2>
      <p>
      ...
      {/* PRAZO */}
      <h2 className="text-sm font-bold mt-6 mb-2">PRAZO</h2>
      <p>O período de prestação de serviço será de <strong>30 dias</strong> a partir da contratação.</p>
      <p className="mt-2">A continuidade poderá ser renovada mediante novo acordo.</p>

      {/* PAGAMENTO */}
      <h2 className="text-sm font-bold mt-6 mb-2">PAGAMENTO</h2>
      <p>Valor total: <strong>R$ {Number(data.valor_total).toFixed(2)}</strong></p>
      <p className="mt-2">Forma de pagamento: <strong>{paymentLabel[data.forma_pagamento]}</strong></p>
      {data.numero_parcelas > 1 && (
        <>
          <p className="mt-2">Parcelamento: <strong>{data.numero_parcelas}x de R$ {valorParcela}</strong></p>
          <p className="mt-2">Vencimento: dia <strong>{data.dia_vencimento}</strong> de cada mês</p>
        </>
      )}
      ...
      {data.desconto_regressivo && (
        <>
          <h2 className="text-sm font-bold mt-6 mb-2">DESCONTO REGRESSIVO</h2>
          <p>O CONTRATANTE poderá realizar quitação antecipada do contrato.</p>
          <p className="mt-2">O desconto será regressivo:</p>
          <ul className="list-disc ml-8 my-2">
            <li>15% inicial</li>
      ...
      {/* LIMITAÇÕES */}
      <h2 className="text-sm font-bold mt-6 mb-2">LIMITAÇÕES</h2>
      ...
      <p className="mt-2">O perfil pode sofrer suspensão conforme políticas do Google.</p>
      <p className="mt-2">O CONTRATADO não se responsabiliza por alterações feitas pelo cliente ou terceiros.</p>
